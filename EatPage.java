...
public class EatPage extends JFrame implements ActionListener {
Calendar cal = Calendar.getInstance();
	...
	JDialog eatInputDialog;
	
	public EatPage(MenuPage menu) {
		super("식단기록");
		this.menu = menu;
	...
	public Color changeBtnColor(int btnNum) {
		...
		String date = dFormat.format(cal.getTime());		//현재날짜(콤보박스에있는)의 연 월		
		String eatDate = date+"%";			
		
		List<EatVO> list = dao.getEatData(eatDate,getLoginId()); // 18/12% 형태로 데이터들불러옴 
		for(int i=0; i<list.size();i++) {
			EatVO vo =list.get(i);								//해당 월에 해당하는 레코드 1줄씩
	...
	public void actionPerformed(ActionEvent e) {				//ACTIONLISTENER
		...
		for(int i =0; i<=dayLength; i++) {  			//날짜버튼 클릭시
			if(obj.equals(String.valueOf(i))) {				
				day = i;							//날짜 저장
													//다이얼로그로 운동인풋창 생성
				eatInputDialog = new JDialog(this,"운동기록", true);
				EatInput ei = new EatInput(this);
				eatInputDialog.add(ei);				
				eatInputDialog.setSize(400,500);
				eatInputDialog.setVisible(true);
				eatInputDialog.setDefaultCloseOperation(DISPOSE_ON_CLOSE);	
				clear();
				dayBtn();  //버튼 다시생성
			}
